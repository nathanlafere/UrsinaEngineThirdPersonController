...
class PerlinNoiseMap(Entity):
    def __init__(self, player, terrain_width, terrain_texture='white_cube', rendering_distance=10, size_render=5):
        super().__init__()
        self.player = player
    ...
    def return_terrain_y(self,x,z):
        return self.noise([(x+self.terrain_width/2+0.5)/self.terrain_width, (z+self.terrain_width/2+.5)/self.terrain_width])*5
